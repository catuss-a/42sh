...
** Last update Tue Dec 14 23:22:21 2010 younes serraj
*/
...
static void		env_entry_update(t_ll_list *list,
					...
					t_config *conf, int *ret)
{
  ...
  if (argc < 3)
    {
      my_puterr("Usage: setenv [name] [value] ([overwrite])\n");
      *ret = -1;
      return (MYSH_SIGNAL_NOPE);
    }
  overwrite = (argc == 4) ? atoi(argv[3]) : 1;
  if (!mem_exists(&(conf->env), argv[1]))
    {
      if (overwrite == 0)
	{
	  ...
	  *ret = -1;
      ...
      env_entry_update(&(conf->env), argv[1], argv[2]);
    }
  else
    ll_push_data(&(conf->env), var_make(argv[1], argv[2]), -1);
  *ret = 0;
  return (MYSH_SIGNAL_NOPE);
}
